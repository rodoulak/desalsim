...
#input data
Qf=sc1.Qconc/sc1.d_out #m3/hr
T_initial =273+25
#%%

#Declaration of constant process parameters - Part 1: Process Parameters

Vreactor=(sc1.Qconc/sc1.d_out)/0.7            #Volume of the reactor : m^3
...
#Net Heat flux 
Cooling_rate=250000                                                                                                 #Assumed NET cooling rate to cool down : W = J/s :Assumption !!! Includes the heat removed by the heat exchanger and the heat provided by the environment and the system
...
#V_ice_distr_store=np.array([])
#V_Na2SO4_cr_distr_store=np.array([])
...
def Compounds():
    ...
    C_so4_in=0.211#(sc1.Cconci[5]/constants.MW_so4)/(sc1.d_out/1000) #mol/kg
    print("C_so4_i is "+str(C_so4_in))
    C_nacl_in=(sc1.Cconci[0]/constants.MW_Na-2*C_so4_in)/(sc1.d_out/1000) #mol/kg
    ...
    #     T_Na2SO4_sol_line.append(row[0])
    #     C_Na2SO4_sol_line.append(row[1])
    T_Na2SO4_sol_line= df.iloc[:, 1].tolist()
    C_Na2SO4_sol_line=df.iloc[:, 0].tolist()
    C_Na2SO4_sol_line=np.array(C_Na2SO4_sol_line)
    # T_Na2SO4_sol_line=pitzer_model_3phases_w.line_na2so4[0]
    # zipped_lists = zip(T_Na2SO4_sol_line, C_Na2SO4_sol_line)
    # sorted_lists = sorted(zipped_lists, key=lambda x: x[0])
    # T_Na2SO4_sol_line, C_Na2SO4_sol_line = zip(*sorted_lists)
    # T_Na2SO4_sol_line.sort()
    print("T_Na2SO4_sol_line "+str(T_Na2SO4_sol_line))
   
    # C_Na2SO4_sol_line.sort()
    print("C_Na2SO4_sol_line "+str(C_Na2SO4_sol_line))
                                                            #Transform the solubility line in mols Na2SO4/ kg water = molals
...
def Initialize(Var):
    ...
    #Var.V_water=Var.V_mother_liquor*(1-Var.C_compound1*Compound1.MW/(Compound1.d)-Var.C_compound2*Compound2.MW/(Compound2.d))       #Volume of water in the reactor : m^3
    Var.V_water=Var.V_mother_liquor/(1+Var.C_compound1*Compound1.MW*d_water/(1000*Compound1.d)+Var.C_compound2*Compound2.MW*d_water/(1000*Compound2.d))       #Volume of water in the reactor : m^3
    ...
    #Var.V_compound1=Var.V_mother_liquor*Var.C_compound1*Compound1.MW/Compound1.d#(Var.C_compound1*Compound1.MW*Var.V_water*d_water)/(1000*Compound1.d)                                                                     #Volume of compound 1 in the reactor : m^3
    #Var.V_compound2=Var.V_mother_liquor-Var.V_water-Var.V_compound1                                                                                           #Volume of compound 2 in the reactor : m^3
...
def MassBalances():    
    ...
    # print("Var.M_compound1 is "+str(Var.M_compound1))   
    # print("Fin_compound1*deltat is "+str(Fin_compound1*deltat))
    # print("Fout_compound1*deltat is "+str(Fout_compound1*deltat))
    # print("M_compound1_cr_reactions    is "+str(M_compound1_cr_reactions))
    print(""+str())
...
def HeatBalances():
    ...
    #Water_weight_percentage=(Var.M_water/Var.M_liquid_tot)*100
    #NaCl_weight_percentage=(Var.M_compound2/Var.M_liquid_tot)*100
    #Ice_weight_percentage=(Var.M_ice/Var.M_liquid_tot)*100
    ...
    #Cp_solution=(Na2SO4_weight_percentage*Cp_Na2SO4)+(Ice_weight_percentage*Cp_ice)+(Water_weight_percentage*Cp_water)+(NaCl_weight_percentage*Cp_NaCl) #Heat capacity of the solution (Calculated for 25 oC, Including the impurities and assuming no change of heat capacity due to temperature) (source:Heat of solution, heat capacity...) :   J/(kg*K)
    #dsol=0.9974+0.002625*Na2SO4_weight_percentage+0.00000401*Na2SO4_weight_percentage**2                                 #Density of the solution (Calculated for 25 oC, Neglecting the impurities and assuming no change of density due to temperature) (source:Heat of solution, heat capacity...) : g/ml
...
def Store():
    global t_store,sigma_compound1_store,DT_ice_store,M_ice_store,M_compound1_cr_store,Treactor_store #,V_ice_distr_store,V_Na2SO4_cr_distr_store
    ...
    #V_ice_distr_store=np.append(V_ice_distr_store,Var.V_ice_distr)
    #V_Na2SO4_cr_distr_store=np.append(V_Na2SO4_cr_distr_store,Var.V_Na2SO4_cr_distr)
...
def Plot():
    global t_store,sigma_compound1_store,DT_ice_store,M_ice_store,M_compound1_cr_store,Treactor_store,graph1,graph2,graph3,graph4 #,V_ice_distr_store,V_Na2SO4_cr_distr_store,
...
class conc_f:
    ...
    def calc_f_c(self):
        ...
        self.Mna_out=self.Cna_in*self.Min/(1000*sc1.d_out/1000)-self.M_compound1_cr2*2*constants.MW_Na/MW_Na2SO4_10H2O
        self.Cna_out=((self.Cna_in*self.Min/(1000*sc1.d_out/1000)-self.M_compound1_cr2*2*constants.MW_Na/MW_Na2SO4_10H2O)/self.Mout)/1000#g/l
        self.Cso4_out=((self.Cso4_in*self.Min/(1000*sc1.d_out/1000)-self.M_compound1_cr2*constants.MW_so4/MW_Na2SO4_10H2O)/self.Mout)/1000#g/l
...
Flow_rate=Qf*(sc1.d_out/1000)/Residence_time
print('Continous Flow Rate : ', Flow_rate,'liters/hour')
Min=sc1.Qconc#Residence_time #kg/hr 
#mass flowrate of na2so4 
M_compound1_cr=Var.M_compound1_cr/res#*MW_Na2SO4_10H2O/MW_Na2SO4/res#Residence_time
...
Mout=(np.asscalar(Var.M_liquid_tot)+308)/res#Residence_time
print("Mass flow rate for effluent is "+ str(Mout)+"kg/hr")
#Cna_in, Ccl_in, Ck_in, Cmg_in, Cca_in, Cso4_in, Qin, Min, Mout
conc_f_in=conc_f(sc1.Cconci[0],sc1.Cconci[1], sc1.Cconci[2], sc1.Cconci[3], sc1.Cconci[4], sc1.Cconci[5],sc1.Qconc/sc1.d_out, sc1.Qconc, Mout, M_compound1_cr)
#conc_f_in=conc_f(sc1.Cconci[0],sc1.Cconci[1], sc1.Cconci[2], sc1.Cconci[3], sc1.Cconci[4], sc1.Cconci[5],sc1.Qconc/sc1.d_out, sc1.Qconc, Mout, M_compound1_cr)
#conc_f_in=conc_f(mfpfr_unit2.Cout_all[0], mfpfr_unit2.Cout_all[1], mfpfr_unit2.Cout_all[2], mfpfr_unit2.Cout_all[3], mfpfr_unit2.Cout_all[4], mfpfr_unit2.Cout_all[5], mfpfr_unit2.Qout_2, mfpfr_unit2.Mout_2, Mout, M_compound1_cr)
dens=1.02
conc_f_in.calc_f_c()
#Cout_efc=[conc_f_in.Cna_out, conc_f_in.Ccl_out, conc_f_in.Ck_out, conc_f_in.Cmg_out, conc_f_in.Cca_out,conc_f_in.Cso4_out] 
...
# Cout_efc=[2*Cout_so4+Cout_cl, Cout_cl, 0, 0, 0, Cout_so4] #mol/l 
# Cout_efc_g=[(Cout_so4/constants.MW_so4*2+Cout_cl)*constants.MW_Na, Cout_cl*constants.MW_cl, conc_f_in.Ck_out, 0, 0, Cout_so4] #g/l 
# print("Cout_efc is "+str(Cout_efc_g))
# Cout_efc_g=[(Cout_so4/constants.MW_so4*2)+Cout_cl*constants.MW_Na*Qf*1000/Mout, Cout_cl*constants.MW_cl*Qf*1000/Mout, conc_f_in.Ck_out*Qf*1000/Mout, 0, 0, Cout_so4] #g/l 
# print("Cout_efc is "+str(Cout_efc_g))
#print("out concentration for na2so4 in remaining solution is "+str(*Cout[0])+" mol/l ")
...
# M_compound1_cr=Var.M_compound1_cr*3600*MW_Na2SO4_10H2O/MW_Na2SO4
# print("M_compound1_cr is "+str(M_compound1_cr))
V_compound1=Var.V_compound1/res#Residence_time#*MW_Na2SO4_10H2O/MW_Na2SO4
...
# print("M_compound1_cr is "+str(M_compound1_cr))
w_in_cryst=M_compound1_cr*10*18.01528/MW_Na2SO4
print(w_in_cryst)
bal=sc1.Qconc-M_ice*res-Mout*res-M_compound1_cr*res
