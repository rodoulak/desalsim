...
#save initial concentration before recirculation: 
    ...
    def flowrate(self):
        self.Q1_s_in=self.Qin  #units: l/h
        self.Q1_b_in=self.Qin  #units: l/h
        self.Q1_a_in=self.Qin  #units: l/h
    ...
    def salt_channel(self):
        ...
        self.EMF=((R_const*T/z/F)*(self.c1_emf+self.c2_emf+self.c3_emf+self.c4_emf)) 
       # self.EMF=(((R_const*T)/z/F)*(math.log(((self.Ci_a_in[7]+self.Ci_a_out[7])/2)/Cm_bp_H)+math.log(Cm_bp_OH/(self.Ci_b_in[8]+self.Ci_b_out[8])/2)+math.log(((self.Ci_s_in[1]+self.Ci_s_out[1])/2)/((self.Ci_a_in[1]+self.Ci_a_out[1])/2))+ math.log(((self.Ci_b_in[0]+self.Ci_b_out[0])/2)/((self.Ci_s_in[0]+self.Ci_s_out[0])/2)))) #incomplete       
        self.V_ext=self.EMF+((self.I_ext*R_int)/(A*10000)) 
...
# N_trip=round(scaleup.scaleup(40, 229, Q_in_edbm),0)       

A=0.19*Q_in_edbm/60#scaleup.scaleup(0.19, 229, Q_in_edbm)#19*Q_in_edbm/60/3#
print("A is "+str(A))
N_trip=40
...
#
while (edbm_dat.Ci_b_out[0]<1) and (edbm_dat.Ci_s_out[0]>=0.15):
...
Q_a_out=edbm_dat.Q_a_out 
print("Q_a_out is "+str(Q_a_out)+"l/hr")
M_HCl_out=Q_a_out*constants.MW_HCl/1000 #kg/hr
Q_b_out=edbm_dat.Q_b_out 
print("Q_b_out is "+str(Q_b_out)+"l/hr")
M_NaOH_out=Q_a_out*constants.MW_NaOH/1000 #kg/hr
Q_s_out=edbm_dat.Q_s_out 
...
Ppump=(3*Q_in_edbm*0.1+Q_s_out*1+Q_b_out*1+Q_a_out*1+edbm_dat.Q_s_r *1.2+edbm_dat.Q_a_r *1.2+edbm_dat.Q_b_r *1.2)/1000/3600*1e5/npump #units: W -> l to m3 so /1000; bar to J 1e5N/m2*1J/m ; hr to 3660s  
...
SEC=E_el_Edbm/M_NaOH_out
print("sec is "+str(SEC)+"kwh/kg naoh")
Q_w_in=2*Q_in_edbm
bal=(Q_in_edbm*d_in+2*Q_in_edbm)-M_a_out-M_s_out-M_b_out#edbm_dat.M_a_t_out -edbm_dat.M_b_t_out -edbm_dat.M_s_t_out +M_s_out_r+M_a_out_r+M_b_out_r
...
sum_table_f={'F1: Qs_in->efc effluent': Q_in_edbm,
           ...
           # 'F3: Qs_out_t': ,
           'F4: Qs_out': Q_s_out, 
           'F5: Qs_r': edbm_dat.Q_s_r ,
           'F6: Qa_in water': Q_in_edbm,
           # 'F7: Qa_in_t': , 
           # 'F8: Qa_out_t': ,
           'F9: Qa_out': Q_a_out, 
            'F10: Qa_r': edbm_dat.Q_s_r ,
           'F11: Qb_in water': Q_in_edbm ,
           # 'F12: Qb_in_t': , 
           # 'F13: Qb_out_t': ,
           'F14: Qb_out': Q_b_out, 
           'F15: Qb_r': edbm_dat.Q_s_r ,
           }
